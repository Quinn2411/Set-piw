...
class visualcards(pygame.sprite.Sprite):
    def __init__(self,cardindexontable,card):
        super().__init__()
        self.image=pygame.image.load('kaarten/'+str(card._colour+card._symbol+card._shading+card._number)+'.gif').convert_alpha()
...
while run:
    ...
    if (isMessageDisplayed):
        ...
        screen.blit(message_text, (SCREEN_WIDTH / 2 - message_text.get_width()/2, SCREEN_HEIGHT/5*4))
